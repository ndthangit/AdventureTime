...
public class EndUI extends Table {
    private final Skin skin;
    private final CoreGame game;
    private Table menu;
    private Table screen;
    private LoadingUI loadingUI;
    public EndUI(Skin skin, CoreGame game) {
        super(skin);
        setFillParent(true);
        this.skin = skin;
        this.game = game;
...
//        menu.setBackground(new TextureRegionDrawable(new TextureRegion(new Texture(Gdx.files.internal("OOP_Project 3 (2).png")))));
    ...
    void createTitle() {
        TextButton title = new TextButton("THE END", skin, "huge");
        screen.add(title).expandX().fillX().padTop(200).top().row();
    }
    void createStartGame() {
        TextButton replayButton = new TextButton("RESTART", skin, "huge");
