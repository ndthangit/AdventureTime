...
public class EnemyMovementSystem extends IteratingSystem {
    ...
    public EnemyMovementSystem(CoreGame game, Engine engine) {
    ...
    protected void processEntity(Entity entity, float deltaTime) {
        PlayerComponent playerCom = ComponentMapper.getFor(PlayerComponent.class).get(getPlayerEntity());
        EnemyComponent enemyCom = em.get(entity);
        Box2DComponent b2dComponent = bodm.get(entity);

        Vector2 playerPos = playerCom.getPosition();
        ...
        if (distance < 3) {
            // Đuổi theo Player
            Vector2 dir = new Vector2(playerPos.x - enemyPos.x, playerPos.y - enemyPos.y);
            dir.nor();
            b2dComponent.body.setTransform(b2dComponent.body.getPosition().x + dir.x * speed.x * deltaTime,
                    b2dComponent.body.getPosition().y + dir.y * speed.y * deltaTime,
            ...
            if (!enemyPos.epsilonEquals(enemyCom.startPosition, 0.1f)) {
                ...
                dir.nor();
                b2dComponent.body.setTransform(b2dComponent.body.getPosition().x + dir.x * deltaTime * 3,
                        b2dComponent.body.getPosition().y + dir.y * deltaTime * 3,
                        b2dComponent.body.getAngle());
            } else {
